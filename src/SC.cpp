...
int main(int argc,char** argv){
    ...
    for(int k=0;k<30;k++){
        ...
        for(int i=0;i<10;i++){
            for(int j=0;j<4;j++){
                layer_1[i] = APC(layer_1[i] ,ESL_Multiplier(layer_0[j],number_gen(weight_1[i][j],all_one)));
            }
            APC(layer_1[i],number_gen(bias_1[i],all_one));
        }
    
        for(int i=0;i<5;i++){
            for(int j=0;j<10;j++){
                layer_2[i] = APC(layer_2[i] ,ESL_Multiplier(layer_1[j],number_gen(weight_2[i][j],all_one)));
            }
            layer_2[i] = APC(layer_2[i],number_gen(bias_2[i],all_one));
            cout<<"layer2 i = "<<i<<' '<<print(layer_2[i])<<endl;
        }
        
        for(int i=0;i<3;i++){
            for(int j=0;j<5;j++){
                layer_3[i] = APC(layer_3[i] ,ESL_Multiplier(layer_2[j],number_gen(weight_3[i][j],all_one)));
                // cout<<"layer_3[i][j] = "<<print(layer_3[i])<<' '<<"weight_3[i][j] = "<<print(number_gen(weight_3[i][j],all_one))<<endl;
                
            }
            // cout<<"layer_3[i] = "<<print(layer_3[i])<<' '<<"bias_3[i] = "<<print(number_gen(bias_3[i],all_one))<<endl;
            layer_3[i] = APC(layer_3[i],number_gen(bias_3[i],all_one));
            // cout<<"i = "<<i<<' '<<print(layer_3[i])<<endl;
        }
        for(int j=0;j<3;j++){
            answer1 = 0;
            answer2 = 0;
