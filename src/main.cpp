...
int main(int argc,char** argv){
    ...
    //set threshold for batch normalization use...
...
float threshold_3[4] = {-2.493149518966675, -7.356795787811279, -7.115196228027344, -2.823887348175049};
    ...
    fc_neurons[0] = new float[64];
    for(size_t i = 0; i < 64; ++i){
        fc_neurons[0][i] = 0;
    }

    fc_neurons[1] = new float[32];
    for(size_t i = 0; i < 32; ++i){
        fc_neurons[1][i] = 0;
    }

    fc_neurons[2] = new float[32];
    for(size_t i = 0; i < 32; ++i){
        fc_neurons[2][i] = 0;
    }

    fc_neurons[3] = new float[4];
    for(size_t i = 0; i < 4; ++i){
        fc_neurons[3][i] = 0;
    }

    fc_neurons[4] = new float[4];
    for(size_t i = 0; i < 4; ++i){
        fc_neurons[4][i] = 0;
    }
    ...
    for(size_t i = 0; i < 3; ++i)
    {
        ...
        for(size_t k = 0; k < 32; ++k)
        {
            // cout << fc_neurons[2][k] << endl;
            if(fc_neurons[2][k] < threshold_2[k]) fc_neurons[2][k] = -1;
            else if(fc_neurons[2][k] > threshold_2[k])fc_neurons[2][k] = 1;
        }

        fc_neurons[3] = sc.linear(fc_neurons[2], fc_weight[2], sum, 32, 4);
        fc_neurons[3][0] -= threshold_3[0];
        for(size_t k = 1; k < 4; ++k){
            fc_neurons[3][k] -= threshold_3[k];
            fc_neurons[3][k] = -fc_neurons[3][k];
        }
        ...
        for(size_t j = 0; j < 4; ++j)
        {
            cout << j << " : " << fc_neurons[3][j] << endl;
            if(fc_neurons[3][j] > max)
            {
                max = fc_neurons[3][j];
                max_cand = j;
            }
